...
2.- <pre><code class="ruby">bundle install</code></pre>
...
<pre><code class="ruby">rails g scaffold authentication user_id:integer provider:string uid:string</code></pre>

7.- Associate users and authentications
<pre><code class="ruby">
user.rb > references_many :authentications, :dependent => :delete
</code></pre>
<pre><code class="ruby">
authentication.rb > referenced_in :user
</code></pre>

8.- Modify *routes.rb* to redirect the authentication callback
<pre><code class="ruby">match '/auth/:provider/callback' => 'authentications#create'</code></pre>
...
<pre><code class="ruby">
  def index    @authentications = current_user.authentications if current_user
  end
...
<pre><code class="ruby">
require 'string_extensions'include StringExtensions
</code></pre>
...
<pre><code class="ruby">rails g controller registrations</code></pre>
...
else      #New user data not valid, try again
      flash[:alert] = t(:fail)
      redirect_to new_user_registration_url
    end
</code></pre>
...
<pre><code class="html">
<h2>Email????</h2><%= form_tag(:controller => "registrations", :action => "email") do %>
...
*app/models/user.rb*
<pre><code class="ruby">
def apply_omniauth(omniauth) => def apply_omniauth(omniauth, confirmation)
</code></pre>
<pre><code class="ruby">
def apply_trusted_services(omniauth)  => def apply_trusted_services(omniauth, confirmation)  
</code></pre>
<pre><code class="ruby">
 self.confirmed_at, self.confirmation_sent_at = Time.now  =>
if (confirmation)      
...
*app/controllers/authentications_controller.rb*
<pre><code class="ruby">
user.apply_omniauth(omniauth) => user.apply_omniauth(omniauth, true)
</code></pre>
...
Daniel Kehoe ("http://fernandotapiarico.com/":http://fernandotapiarico.com/)
